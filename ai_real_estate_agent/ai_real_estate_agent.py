...
class PropertyData(BaseModel):
    ...
    building_name: str = Field(
        description="Name of the building/property", alias="Building_name"
    )
    ...
    description: str = Field(
        description="Detailed description of the property", alias="Description"
    )
...
class PropertyFindingAgent:
    ...
    def __init__(
        self, firecrawl_api_key: str, gemini_api_key: str, model_id: str = "o3-mini"
    ):
        self.agent = Agent(
    ...
    def find_properties(
        ...
        property_type_prompt = (
            "Flats" if property_type == "Flat" else "Individual Houses"
        )
...
def main():
    ...
    with st.sidebar:
        ...
        model_id = st.selectbox(
            "Choose GeminiAI Model",
            options=["gemini-1.5-flash", "gemini-1.5-flash-8b", "gemini-2.0-flash-exp"],
            help="Select the AI model to use. Choose Gemini-2.0.",
        )
        ...
        firecrawl_key = st.text_input(
            "Firecrawl API Key", type="password", help="Enter your Firecrawl API key"
        )
        ...
        try:
            with st.spinner("🔍 Searching for properties..."):
                ...
                with st.spinner("📊 Analyzing location trends..."):
                    location_trends = (
                        st.session_state.property_agent.get_location_trends(city)
                    )
